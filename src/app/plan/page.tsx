...
function ApprovedPlan({
  ...
  return (
    <div className="page-wrap">
      ...
      {criticActions.length > 0 && (
        <section className="card card-pad plan-risks">
          <div className="card-head">
            <div>
              <div className="card-note">Campaign Critic</div>
              <h2 className="section-title" style={{ marginTop: 5 }}>Actions added from campaign reviews</h2>
            </div>
            <Sparkles size={15} />
          </div>
          <div className="plan-alternatives">
            {criticActions.map((action) => (
              <div key={action.id}>
                <strong>{action.hook}</strong>
                <span>{action.channel} · {action.format} · {action.pillar}</span>
...
export default async function PlanPage() {
  ...
  let criticActions: CriticAction[] = [];
  let failure = "";
  try {
    ...
    if (brand) {
      const actionPlan = await getDb().plan.findFirst({
        where: { brandId: brand.id },
        orderBy: { weekOf: "desc" },
        include: { items: { where: { status: "idea" } } },
      });
      criticActions = actionPlan?.items ?? [];
    }
  } catch (error) {
  ...
  return campaign
    ? <ApprovedPlan campaign={campaign} criticActions={criticActions} />
    : <EmptyPlan />;
}
